...
public:
  ...
  void synthSelect(std::string synthOptions[], int chosenSynth, Synth *mySynths[])
  {
    ...
    if (synthOptions[chosenSynth] == "both")
    {
      ...
      mySynths[1] = new Vibe_Synth;
      freq4 = ui.retrieveDoubleInRange("The vibe synth has a wobble\nEnter a Frequency for the wobble between: ", 1, 100);
      detuneOsc4 = ui.retrieveFloatInRange("The add synth has 3 extra oscillators which can be detuned\nEnter the amount of detuning between: ", 0, 100);
  ...
  void prepare(double sampleRate) override
  {
    ...
    if (melodyAmount == 0)
    {
      ...
      timeDivisionchoice = ui.retrieveSelection("Enter a time division for the melody choose between: ", timeDivisionOptions,4);
      // if there is one melody the time division is equal for both.
      if (timeDivisionchoice == 0)
      {
        timeDivision = 120;
      }
      else if (timeDivisionchoice == 1) 
      {
        timeDivision = 60;
      }
      else if (timeDivisionchoice == 2) 
      {
        timeDivision = 30;
      }
      else if (timeDivisionchoice == 3) 
      {
        timeDivision = 15;
      }
    ...
    else if (melodyAmount == 1)
    {
      ...
      // choosing scale based on selected scale
      melody.chooseScale(melody.scaleOptions, scale, melody);
      melody2.chooseScale(melody2.scaleOptions, scale, melody2);
      timeDivisionchoice = ui.retrieveSelection("Enter a time division for Melody 1 choose between: ", timeDivisionOptions, 4);
      timeDivisionchoice2 = ui.retrieveSelection("Enter a time division for Melody 2 choose between: ", timeDivisionOptions, 4);
      // selecting the right timeDivision based on user input. 
      if (timeDivisionchoice == 0)
      {
        timeDivision = 120;
      }
      ...
      if (timeDivisionchoice2 == 0)
      {
        timeDivision2 = 120;
      }
      else if (timeDivisionchoice2 == 1) 
      {
        timeDivision2 = 60;
      }
      else if (timeDivisionchoice2 == 2) 
      {
        timeDivision2 = 30;
      }
      else if (timeDivisionchoice2 == 3) 
      {
        timeDivision2 = 15;
      }
    ...
    amplitudeL = ui.retrieveFloatInRange("Enter a amplitude for the Left channel between: ", 0.1, 1);
    amplitudeR = ui.retrieveFloatInRange("Enter a amplitude for the Right channel between: ", 0.1, 1);
...
protected:
  ...
  // options different time divisions for different melodies
  std::string timeDivisionOptions[4] = {"1/2","1/4","1/8","1/16"};
  int timeDivisionchoice = 0;
  int timeDivisionchoice2 = 0;
